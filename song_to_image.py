...
normalized_data = ((raw_data - raw_data.min()) / (raw_data.max() - raw_data.min()) * 255).astype(np.uint8)
...
padded_data = np.concatenate((normalized_data, np.zeros(side_len**2 - len(normalized_data), dtype=np.uint8)))
